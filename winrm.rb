...
def check_directories(args)
  if(File.directory?(args))
    ...
    puts("The directory was not found " + args)
    abort
  end
end
...
def rutas(args)
  directorio = args
  files = Dir.glob("#{directorio}*.*",File::FNM_DOTMATCH )
  directorios = Dir.glob("#{directorio}*").select {|f| File.directory? f}
  resultado = files + directorios
  return resultado
end

check_directories(scripts)
check_directories(executables)
funciones = lee_scripts(scripts)
executables = lee_executables(executables)
...
conn.shell(:powershell) do |shell|
    until command == "exit" do
        ...
        if command.start_with?('upload') then
            ...
            command = ""
            # Si el fichero a subir esta en el directorio actual no es necessario indicar el nombre de subida
            # Pero si hemos tabulado si debemos hacerlo 
            ...
            command = ""
            # Si el fichero a descargar esta en el directorio actual no es necesario indicar el nombre de descarga
            # Pero si tiene ruta parcial o absoluta si debemos hacerlo
        ...
        elsif command.start_with?(*funciones) then
            silent_warnings do
            ...
            output = shell.run("Menu")
            autocompleta = shell.run("auto").output.chomp
            autocompleta = autocompleta.gsub!(/\r\n?/, "\n")
            LIST2 = autocompleta.split("\n")
            LIST = LIST + LIST2
            print(output.output)
            end    
        end
