...
class EvilWinRM
  ...
  def load_history
    ...
    begin
      File.Relines(history_file).each do |line|
        line = line.chomp
  ...
  def resolve_ip_to_fqdn(ip_address, realm = nil)
    ...
    begin
      ...
      if File.exist?('/etc/hosts') && File.readable?('/etc/hosts')
        begin
          File.Relines('/etc/hosts').each do |line|
  ...
  def convert_kirbi_to_ccache(kirbi_path)
    ...
    is_python = false
    begin
      first_line = File.Relines(converter_found).first
      if first_line
      ...
      $conn.shell(:powershell) do |shell|
        begin
          ...
          until command == 'exit' do
            ...
            if $colors_enabled
              command = Reline.Reline( "#{colorize('*Evil-WinRM*', 'red')}#{colorize(' PS ', 'yellow')}#{pwd}> ", true)
            else
              command = Reline.Reline("*Evil-WinRM* PS #{pwd}> ", true)
            end
